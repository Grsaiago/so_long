...
/*   Updated: 2022/09/23 14:10:56 by gsaiago          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	initialize_images(t_data *s_data)
{
	...
	s_data->i_enemy = mlx_xpm_file_to_image(s_data->mlx_ptr,
			"./assets/enemy.xpm", &s_data->i_height, &s_data->i_width);
...
int	paint_bg(t_data *s_data)
{
	...
	s_data->k_str = ft_itoa(s_data->k_count);
	mlx_string_put(s_data->mlx_ptr, s_data->win_ptr,
		45, 45, 500, s_data->k_str);
	free(s_data->k_str);
	return (0);
}
...
void	img_put(t_data *s_data, char c, int i, int j)
{
	if (c == '0')
		...
		mlx_put_image_to_window(s_data->mlx_ptr, s_data->win_ptr,
			s_data->i_enemy, s_data->pos_x, s_data->pos_y);
	else if (c == 'P')
	{
		mlx_put_image_to_window(s_data->mlx_ptr, s_data->win_ptr,
